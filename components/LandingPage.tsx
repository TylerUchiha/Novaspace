...
import { Building2, ArrowRight, User, ShieldCheck, Mail, Lock, UserPlus, Phone, Map, Layout, Calendar, CheckCircle2, Play, MousePointer2, Sparkles, ArrowDown, Globe, X, Crown } from 'lucide-react';
import { motion, useInView, AnimatePresence } from 'motion/react';
import { useAuth } from "./AuthProvider";

interface LandingPageProps {
  onLogin: (role: 'customer' | 'employee' | 'owner' | 'manager') => void;
...
const LandingPage: React.FC<LandingPageProps> = ({ onLogin, onCodeLogin, onShowPrivacy, onShowTerms, onShowSupport }) => {
        ...
        <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-12 items-center relative z-10">
          ...
          >
            ...
            <div className="bg-white rounded-[3.5rem] shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] border border-slate-100 overflow-hidden flex flex-col min-h-[560px]">
              ...
              {authError && (
                <div className="bg-rose-50 border border-rose-200 text-rose-700 p-4 rounded-xl flex items-start justify-between">
                  <p className="text-sm font-bold flex-1">{authError}</p>
                  <button onClick={clearAuthError} className="text-rose-400 hover:text-rose-600 ml-3">
                    <X size={18} />
                  </button>
                </div>
              )}
              ...
              <form className="space-y-5" onSubmit={handleSubmit}>
                {activeTab === 'employee' ? (
                  <div className="space-y-6">
                    ...
                    <button 
                      type="submit"
                      className="w-full py-5 rounded-2xl font-black text-lg text-white bg-emerald-600 shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all active:scale-[0.98] mt-2 flex items-center justify-center gap-3"
                    >
                  ...
                  <>
                    ...
                    <button 
                      type="submit"
                      className="w-full py-5 rounded-2xl font-black text-lg text-white bg-blue-600 shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all active:scale-[0.98] mt-2 flex items-center justify-center gap-3"
                    >
                      {activeTab === 'employee' ? 'Admin Access' : (isSignUp ? 'Create My Account' : 'Sign In')}
                      <ArrowRight size={22} />
                    </button>
                  </>
                )}
              ...
              {activeTab === 'customer' && (
                <div className="mt-8 flex flex-col gap-6">
                  ...
                  <button 
                    ...
                    className="w-full py-5 px-6 rounded-2xl bg-white border border-slate-200 text-slate-700 font-black text-sm flex items-center justify-center gap-4 hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm active:scale-[0.98]"
                  >
                    <GoogleIcon />
