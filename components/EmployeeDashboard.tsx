...
interface EmployeeDashboardProps {
  ...
  onVerifyInstapay?: (ids: string[]) => void;
  userProfile: UserProfile;
  allUsers: UserProfile[];
  cancellationPolicy?: string;
  selectedVendor: Vendor | null;
  filterStatus?: 'pending' | 'approved' | 'cancelled';
}
...
const EmployeeDashboard: React.FC<EmployeeDashboardProps> = ({ 
  ...
  const [verifyingInstapayGroup, setVerifyingInstapayGroup] = useState<Reservation[] | null>(null);
  ...
  const groupedReservations = useMemo(() => {
    ...
    const filtered = reservations.filter(res => {
      if (filterStatus !== 'cancelled' && res.date !== today) return false;
  ...
  }, [reservations, searchTerm, filterLocation, allUsers, selectedCancelledDate, filterStatus]);

  const tableReservations = useMemo(() => {
    return groupedReservations.filter(group => {
      if (group[0].duration <= 0) return false;
  ...
  }, [groupedReservations, filterStatus]);
      ...
      >
        ...
        <div className="flex items-center justify-end border-t border-slate-100/80 pt-2.5 mt-0.5">
          <div className="flex items-center gap-1.5">
            {firstResStatus !== 'confirmed' && (firstRes.paymentMethod !== 'instapay' || firstRes.instapayDetails?.verifiedAt) && (
              <button
  ...
  return (
    <div className="flex-1 flex flex-col p-10 overflow-auto bg-slate-50/30">
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start h-full pb-10">
        ...
        <div className={`${filterStatus === 'cancelled' ? 'xl:col-span-12' : 'xl:col-span-7'} flex flex-col gap-6`}>
            ...
            <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="relative w-full md:w-80 shrink-0 z-20">
                ...
                <input 
                  ...
                  className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
                />
              </div>
              
              {groupedReservations.length > itemsPerPage && (
                <div className="flex items-center gap-4 bg-slate-50 p-1.5 rounded-2xl border border-slate-200 shadow-sm transition-all shrink-0">
                  <button 
                    disabled={currentPage === 0}
                    onClick={() => setCurrentPage(prev => Math.max(0, prev - 1))}
                    className="p-2 rounded-xl hover:bg-white disabled:opacity-20 transition-all text-slate-600 active:scale-95 shadow-sm hover:shadow"
                  >
                    <ChevronLeft size={20} />
                  </button>
                  <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 select-none px-2">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-50">
                  {currentItems.length > 0 ? (
                        ...
                        <tr key={firstRes.id} className="group hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => { 
                              ...
                              <div>
                                ...
                                <div className="flex flex-col gap-0.5 mt-0.5">
                                  <div className="flex items-center gap-2">
                                    <span className={`text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-widest ${status === 'approved' ? 'bg-emerald-100 text-emerald-700' : status === 'pending' ? 'bg-amber-100 text-amber-700' : status === 'resolved' ? 'bg-blue-100 text-blue-700' : 'bg-rose-100 text-rose-700'}`}>{status}</span>
                                    ...
                                    <>
                                      ...
                                      {inStorePrice > 0 && (
                                        <div className="flex flex-col gap-1 mt-1">
                                          ...
                                          <div className="flex flex-col pl-2 border-l-2 border-slate-100 gap-0.5">
                                            {resGroup.flatMap(r => r.selectedMenuItems || []).map((item, idx) => {
                                              const itemName = getMenuItemName(firstRes, item.itemId);
                                              return (
                                                <span key={idx} className="text-[9px] font-semibold text-slate-500 uppercase tracking-wider">
                          ...
                          <td className="px-8 py-6 cursor-default">
                            <button 
                              onClick={(e) => {
                                e.stopPropagation();
                                if (firstRes.paymentMethod === 'instapay') {
                                  setVerifyingInstapayGroup(resGroup);
                                }
                              }}
                              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg font-black text-[10px] uppercase tracking-widest border transition-colors ${firstRes.paymentMethod === 'instapay' ? 'bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-100' : firstRes.paymentMethod === 'credits' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-purple-50 text-purple-600 border-purple-100'}`}
                            >
                               {firstRes.paymentMethod === 'credits' ? <Coins size={14} /> : <CreditCard size={14} />}
                               {firstRes.paymentMethod === 'instapay' ? 'Instapay' : firstRes.paymentMethod === 'credits' ? 'Nova Credit' : 'Prepaid'}
                            </button>
                          </td>
                            ...
                            <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                              {filterStatus === 'cancelled' ? (
                                ...
                                <div className="flex items-center gap-1">
                                  {isApproved ? (
                                    <div 
                                      className="p-2 rounded-xl text-emerald-600 bg-emerald-50 flex items-center justify-center" 
                                      title="Approved"
                                    >
                                      <CheckCircle size={20} />
                                    </div>
                                  ...
                                  <button 
                                    ...
                                    className="p-2 rounded-xl text-rose-500 hover:bg-rose-50 active:scale-95 transition-all" 
                    ...
                    <tr><td colSpan={5} className="px-8 py-20 text-center"><div className="flex flex-col items-center justify-center text-slate-300"><Search size={48} className="mb-4 opacity-20" /><p className="text-xl font-black text-slate-400">No reservations found</p></div></td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <div className="xl:col-span-5 bg-white rounded-[2.5rem] border border-slate-200 p-6 flex flex-col relative max-h-[85vh] shadow-xl overflow-hidden">
            <div className="mb-6 pb-4 border-b border-slate-100 flex items-center justify-between shrink-0">
              <h3 className="text-sm font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                ...
                {filterStatus === 'pending' ? 'Pending Orders' : 'Confirmed Orders'}
              </h3>
              <span className="text-[10px] font-black bg-blue-50 text-blue-600 border border-blue-100 px-3 py-1 rounded-full uppercase tracking-wider">
                {filterStatus === 'pending' ? pendingOrders.length : confirmedOrders.length} {(filterStatus === 'pending' ? pendingOrders.length : confirmedOrders.length) === 1 ? 'Order' : 'Orders'}
              </span>
            </div>
      ...
      {/* Add-ons Modal */}
      {verifyingInstapayGroup && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl p-10 text-center animate-in zoom-in-95 duration-300">
             <h3 className="text-3xl font-black text-slate-900 tracking-tight mb-2">Instapay Verification</h3>
             <p className="text-slate-500 font-bold mb-6">
               Please verify the receipt from <span className="text-slate-900">{verifyingInstapayGroup[0].userName}</span>.
             </p>
             
             {verifyingInstapayGroup[0].instapayDetails?.receiptImage ? (
               <div className="mb-8">
                 <img src={verifyingInstapayGroup[0].instapayDetails.receiptImage} alt="Receipt" className="w-full max-h-[300px] object-contain bg-slate-50 rounded-2xl border border-slate-100 p-2" />
                 {verifyingInstapayGroup[0].instapayDetails.payerAddress && (
                   <p className="mt-4 text-[11px] font-bold text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-100">
                     <span className="uppercase tracking-widest text-slate-400 text-[9px] block mb-1">Payer Instapay Address</span>
                     {verifyingInstapayGroup[0].instapayDetails.payerAddress}
                   </p>
                 )}
               </div>
             ) : (
               <div className="mb-8 p-12 bg-slate-50 border border-slate-100 border-dashed rounded-3xl flex flex-col items-center justify-center">
                 <p className="text-sm font-bold text-slate-400">User hasn't uploaded a receipt yet.</p>
               </div>
             )}
             
             <div className="flex gap-4">
                <button onClick={() => setVerifyingInstapayGroup(null)} className="flex-1 py-4 bg-slate-50 text-slate-400 font-black rounded-2xl uppercase tracking-widest text-xs hover:bg-slate-100 transition-colors">Close</button>
                <button 
                  onClick={() => {
                    if (onVerifyInstapay) onVerifyInstapay(verifyingInstapayGroup.map(r => r.id));
                    setVerifyingInstapayGroup(null);
                  }} 
                  disabled={!verifyingInstapayGroup[0].instapayDetails?.receiptImage || !!verifyingInstapayGroup[0].instapayDetails?.verifiedAt}
                  className="flex-[2] py-4 bg-emerald-600 text-white font-black rounded-2xl uppercase tracking-widest text-xs shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:shadow-none"
                >
                  {verifyingInstapayGroup[0].instapayDetails?.verifiedAt ? 'Already Verified' : 'Verify Payment'}
                </button>
             </div>
          </div>
        </div>
                 ...
                 <div>
                   ...
                   <div className="flex items-center gap-3">
                     ...
                     <div>
                       ...
                       <div className="mt-1">
                         <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-widest ${viewingAddonsFor.status === 'approved' ? 'bg-emerald-100 text-emerald-700' : viewingAddonsFor.status === 'pending' ? 'bg-amber-100 text-amber-700' : viewingAddonsFor.status === 'resolved' ? 'bg-blue-100 text-blue-700' : 'bg-rose-100 text-rose-700'}`}>{viewingAddonsFor.status}</span>
                       </div>
                     </div>
                   </div>
                 </div>
