...
import { Plus, Trash2, Move, Edit3, Settings2, Users, ChevronDown, Sparkles, X, Calendar, MapPin, Layers3, Maximize2, Undo2, Redo2, ArrowRight, Layers, Layout, Globe, Clock, Building2, Info, Map as MapIcon, PlusCircle, Ban, FileText, AlertCircle, Image as ImageIcon, Upload, Trash, ShieldCheck, Mail, Phone } from 'lucide-react';
...
interface PropertyConfigProps {
  ...
  onAddLocation: () => void;
  view?: 'layout' | 'brand' | 'branch' | 'tags' | 'general' | 'space_properties';
  onViewChange?: (view: 'layout' | 'brand' | 'branch' | 'tags' | 'general' | 'space_properties') => void;
...
const PropertyConfig: React.FC<PropertyConfigProps> = ({ 
  ...
  const handleUpdateField = (field: keyof Room, value: any) => {
    ...
    const newRooms = rooms.map(room => {
      if (selectedRoomIds.includes(room.id)) {
        ...
        if (field === 'capacity' && updated.type === 'SharedDesk') {
          updated.totalSeats = value;
        }
        if (field === 'type' && value === 'SharedDesk' && updated.totalSeats === undefined) {
          updated.totalSeats = updated.capacity;
        }
        return updated;
      }
      return room;
  ...
  const handleAddRoom = (type: 'reservable' | 'service' | 'shared') => {
    const roomName = type === 'reservable' ? 'New Space' : (type === 'shared' ? 'Shared Space' : 'Service Area');
    ...
    const newRoom: Room = {
      id: `room-${Date.now()}`,
      name: roomName,
      type: type === 'reservable' ? 'Meeting' : (type === 'shared' ? 'SharedDesk' : 'Service'),
      capacity: type === 'reservable' ? 4 : (type === 'shared' ? 4 : 0),
      totalSeats: type === 'shared' ? 4 : undefined,
      ...
      width: 15,
      height: 15,
      pricePerHour: type === 'reservable' ? 25 : (type === 'shared' ? 15 : 0),
  ...
  const handleMouseDown = (e: React.MouseEvent, room: Room) => {
    ...
    if (isOwner && externalView === 'layout') {
      setIsDragging(true);
      setIsResizing(false);
    }
  ...
  const getBlockStyle = (room: Room) => {
    ...
    const draggingClass = externalView === 'space_properties' 
      ? 'transition-all duration-200 cursor-pointer' 
      : (isCurrentlyActive ? 'transition-none shadow-2xl scale-[1.03] opacity-80 cursor-grabbing' : 'transition-all duration-200 cursor-grab');
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden relative">
      {!hideHeader && (
        <div className="h-16 bg-white border-b border-slate-200 flex items-center px-10 gap-2 shrink-0">
           ...
           {isOwner && (
             <>
               <button 
                 onClick={() => onViewChange?.('space_properties')} 
                 className={`h-full flex items-center gap-2 px-4 border-b-2 font-black text-[10px] uppercase tracking-widest transition-all ${externalView === 'space_properties' ? 'border-emerald-500 text-emerald-600 bg-emerald-50/50' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
               >
                 <Edit3 size={14} />
                 Space Properties
               </button>
      ...
      <div className="flex-1 flex overflow-hidden">
        {(externalView === 'layout' || externalView === 'space_properties') ? (
          <>
              ...
              <div className="px-10 py-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="flex flex-col gap-0.5">
                  <h2 className="text-2xl font-black text-slate-900 tracking-tighter">
                    {externalView === 'layout' ? 'Blueprint Editor' : 'Space Properties'}
                  </h2>
                  <div className="flex items-center gap-2">
                    <div className="group flex items-center px-3 py-1.5 bg-white border border-slate-200 rounded-xl shadow-sm text-[11px] font-black text-slate-900">
                    ...
                    <div className="relative">
                        ...
                        {isAddMenuOpen && (
                            <div className="absolute right-0 mt-2 w-56 bg-white border border-slate-200 rounded-2xl shadow-2xl py-2 z-50 overflow-hidden flex flex-col p-1.5 animate-in fade-in slide-in-from-top-2">
                                ...
                                <button onClick={() => handleAddRoom('shared')} className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-blue-50 text-blue-700 font-black text-xs transition-all text-left"><Users size={14} />Shared Space</button>
                            </div>
                        )}
                    </div>
                  )}
                ...
                {rooms.map((room) => {
                  ...
                  return (
                    <div key={room.id} onMouseDown={(e) => handleMouseDown(e, room)} className={`${getBlockStyle(room)} group`} style={{ left: `${room.x}%`, top: `${room.y}%`, width: `${room.width}%`, height: `${room.height}%` }}>
                      ...
                      {isSelected && externalView === 'layout' && <div onMouseDown={(e) => handleResizeStart(e, room)} className="absolute bottom-1 right-1 p-1 bg-emerald-500 text-white rounded-lg cursor-nwse-resize hover:scale-110 transition-all shadow-md"><Maximize2 size={12} strokeWidth={3} className="rotate-90" /></div>}
                    </div>
                  );
                })}
              </div>
            ...
            {selectedRoomIds.length > 0 && externalView === 'space_properties' && (
              <aside className="w-[420px] bg-white border-l border-slate-200 p-10 flex flex-col overflow-y-auto animate-in slide-in-from-right duration-300">
                ...
                <div className="space-y-6">
                    <div className="flex flex-col gap-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Room Name</label><input type="text" readOnly={!isOwner} value={isMultiSelect ? "" : (selectedRooms[0]?.name || "")} placeholder={isMultiSelect ? "Mixed Names" : ""} onChange={(e) => handleUpdateField('name', e.target.value)} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold text-slate-900 outline-none focus:border-emerald-500 focus:bg-white transition-all" /></div>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex flex-col gap-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Type</label><select disabled={!isOwner} value={isMultiSelect ? "" : (selectedRooms[0]?.type || "Meeting")} onChange={(e) => handleUpdateField('type', e.target.value)} className="w-full px-5 py-[13px] bg-slate-50 border border-slate-100 rounded-xl font-bold text-slate-900 outline-none focus:border-emerald-500 focus:bg-white transition-all"><option value="Meeting">Meeting Room</option><option value="Private">Private Office</option><option value="Lounge">Lounge Area</option><option value="HotDesk">Hot Desk</option><option value="SharedDesk">Shared Space</option><option value="Service">Service Area</option></select></div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="flex flex-col gap-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Capacity (Pax)</label><input type="number" readOnly={!isOwner} value={isMultiSelect ? "" : (selectedRooms[0]?.capacity || 0)} onChange={(e) => handleUpdateField('capacity', Number(e.target.value))} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold text-slate-900 outline-none focus:border-emerald-500 focus:bg-white transition-all" /></div>
                      <div className="flex flex-col gap-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Finishing Time</label><input type="text" readOnly={!isOwner} value={isMultiSelect ? "" : (selectedRooms[0]?.finishesAt || "11:00 PM")} placeholder="e.g. 11:00 PM" onChange={(e) => handleUpdateField('finishesAt', e.target.value)} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-xl font-bold text-slate-900 outline-none focus:border-emerald-500 focus:bg-white transition-all" /></div>
                    </div>
                    ...
                    <section className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                       ...
                       <div className="space-y-6">
                          ...
                          <div className="border-t border-slate-100 pt-6 mt-6 space-y-6">
                             <h4 className="font-black text-xs text-slate-900 uppercase tracking-widest">Branch-Specific Contact details</h4>
                             <p className="text-xs text-slate-400 font-bold -mt-2">Leave these empty to inherit from the main co-working space network.</p>
