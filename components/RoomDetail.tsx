...
import { Room, RoomStatus, UserProfile, Vendor, MenuItem, LocationData } from '../types';
...
const RoomDetail: React.FC<RoomDetailProps> = ({ rooms, onBook, selectedDate, selectedTime, selectedTime24, isStaff = false, allUsers = [], userProfile, reservationTimer = null, cancellationPolicy, vendor, currentLocation, basket = {}, basketComments = {}, basketDeliveryTimes = {}, totalOrderComment = "", menuPrice = 0, currentMenu, getBasketItemDetails, onRemoveRoom, onUpdateProfile, bookingDuration, onDurationChange }) => {
  const [isConfirmOpen, setIsConfirmOpen] = useState(false);
  const [paymentMethod, setPaymentMethod] = useState<'card' | 'credits' | 'instapay'>('card');
  
  // Calculate pricing first so it is available for payment selection states
  ...
  const isMethodAllowed = (method: 'card' | 'credits' | 'instapay') => {
    const settingKey = method === 'credits' ? 'novaPoints' : method;
    
    // Check if the current location explicitly has a setting
    if (vendor?.acceptedPaymentMethods) {
      if (vendor.acceptedPaymentMethods[settingKey] === false) {
        return false;
      }
    }
  ...
  useEffect(() => {
    if (isStaff && staffMode === 'select') {
      setPaymentMethod('credits');
      return;
    }
    const allowed: ('card' | 'credits' | 'instapay')[] = [];
    if (isMethodAllowed('card')) allowed.push('card');
    
    // Check if credits are allowed based on selected user context
    const creditsAllowed = isMethodAllowed('credits') && (
      isStaff 
        ? (staffMode === 'select' && !!selectedStaffUser && selectedStaffUser.credits >= totalPrice)
        : (!!userProfile && userProfile.credits >= totalPrice)
    );
    if (creditsAllowed) allowed.push('credits');
    
    if (isMethodAllowed('instapay')) allowed.push('instapay');
    
    if (allowed.length > 0 && !allowed.includes(paymentMethod)) {
      setPaymentMethod(allowed[0]);
    }
  }, [vendor, currentLocation, paymentMethod, isStaff, staffMode, selectedStaffUser, userProfile, totalPrice]);
      ...
      <div className="flex-1 overflow-y-auto space-y-8 custom-scrollbar">
           ...
           <div className="space-y-4">
             ...
             <div className="flex justify-center gap-3 w-full">
                ...
                <button 
                  ...
                  className={`w-24 p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${paymentMethod === 'card' ? 'border-blue-600 bg-blue-50/50 text-blue-600' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                >
                  <CreditCard size={20} />
                ...
                <button 
                  ...
                  className={`w-24 p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${paymentMethod === 'instapay' ? 'border-amber-500 bg-amber-50 text-amber-600' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                >
                  <Banknote size={20} />
                ...
                <button 
                  ...
                  className={`w-24 p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed ${paymentMethod === 'credits' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                >
                  <Coins size={20} />
         ...
         {isStaff && (
            <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
               ...
               {/* Staff Payment Method Selection */}
               <div className="space-y-4 pt-6 border-t border-slate-100 animate-in fade-in">
                 <div className="flex items-center justify-between">
                   <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Payment Method</h3>
                   {staffMode === 'select' && selectedStaffUser && (
                     <div className="flex items-center gap-2 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg animate-in fade-in">
                       <Sparkles size={10} />
                       <span className="text-[10px] font-black uppercase tracking-widest">{selectedStaffUser.credits.toLocaleString()} EGP Balance</span>
                     </div>
                   )}
                 </div>
                 <div className="flex justify-center gap-3 w-full">
                   {isMethodAllowed('card') && (
                    <button 
                      type="button"
                      onClick={() => setPaymentMethod('card')}
                      className={`w-24 p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${paymentMethod === 'card' ? 'border-indigo-600 bg-indigo-50/50 text-indigo-600 font-black' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                    >
                      <CreditCard size={20} />
                      <span className="text-[10px] font-black uppercase tracking-widest text-center">Card</span>
                    </button>
                   ...
                   {isMethodAllowed('instapay') && (
                    <button 
                      type="button"
                      onClick={() => setPaymentMethod('instapay')}
                      className={`w-24 p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${paymentMethod === 'instapay' ? 'border-amber-500 bg-amber-50 text-amber-600 font-black' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                    >
                      <Banknote size={20} />
                      <span className="text-[10px] font-black uppercase tracking-widest text-center">Instapay</span>
                    </button>
                   ...
                   {isMethodAllowed('credits') && (
                    <button 
                      ...
                      className={`w-24 p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed ${paymentMethod === 'credits' ? 'border-emerald-500 bg-emerald-50 text-emerald-600 font-black' : 'border-slate-100 text-slate-400 hover:border-slate-200'}`}
                    >
                      <Coins size={20} />
                 ...
                 {staffMode === 'select' && selectedStaffUser && selectedStaffUser.credits < totalPrice && paymentMethod === 'credits' && (
                   <p className="text-[10px] font-bold text-rose-500 text-center animate-in fade-in">Insufficient credits for this booking.</p>
                 )}
               </div>
            </div>
         )}
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[3rem] shadow-2xl p-10 text-center animate-in zoom-in-95">
             ...
             <div className="mb-6 p-4 bg-slate-50 rounded-2xl flex items-center justify-between">
               <div className="text-left">
                 ...
                 <p className="text-sm font-black text-slate-900 uppercase tracking-tight">{paymentMethod === 'card' ? 'Card' : paymentMethod === 'instapay' ? 'Instapay' : 'Nova Balance'}</p>
               </div>
