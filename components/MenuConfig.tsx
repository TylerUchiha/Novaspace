...
import { Eye, EyeOff, Plus, Minus, Search, CheckCircle, Coffee, User, CreditCard, Coins, Banknote, MapPin, X, Edit, Trash2, Save, Image as ImageIcon } from 'lucide-react';
...
interface MenuConfigProps {
  ...
  userRole: 'customer' | 'employee' | 'owner' | 'manager';
...
const MenuConfig: React.FC<MenuConfigProps> = ({ vendor, location, onUpdateLocation, userRole, allUsers = [], allReservations = [], onBook, onAppendToReservation }) => {
  ...
  const [orderPaymentMethod, setOrderPaymentMethod] = useState<'card' | 'credits' | 'instapay'>('instapay');
  ...
  useEffect(() => {
    if (userMode === 'select') {
      setOrderPaymentMethod('credits');
      return;
    }
    const allowed: ('card' | 'credits' | 'instapay')[] = [];
    if (isMethodAllowed('card')) allowed.push('card');
    if (isMethodAllowed('credits')) allowed.push('credits');
    if (isMethodAllowed('instapay')) allowed.push('instapay');
    
    if (allowed.length > 0 && !allowed.includes(orderPaymentMethod)) {
      setOrderPaymentMethod(allowed[0]);
    }
  }, [location, orderPaymentMethod, userMode]);
  ...
  if (userRole === 'owner' || userRole === 'manager') {
    return <MenuManager location={location} onUpdateLocation={onUpdateLocation} userRole={userRole} />;
  }
                ...
                <div className="space-y-3 pt-6 border-t border-slate-100">
                  ...
                  <div className="flex justify-center gap-2 w-full">
                    ...
                    <button 
                      ...
                      className={`w-24 p-3 rounded-xl border transition-all flex flex-col items-center gap-2 ${orderPaymentMethod === 'card' ? 'border-blue-600 bg-blue-50/50 text-blue-600 font-black' : 'border-slate-100 text-slate-400'}`}
                    >
                      <CreditCard size={16} />
                    ...
                    <button 
                      ...
                      className={`w-24 p-3 rounded-xl border transition-all flex flex-col items-center gap-2 ${orderPaymentMethod === 'instapay' ? 'border-amber-600 bg-amber-50 text-amber-600 font-black' : 'border-slate-100 text-slate-400'}`}
                    >
                      <Banknote size={16} />
                    ...
                    <button 
                      ...
                      className={`w-24 p-3 rounded-xl border transition-all flex flex-col items-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed ${orderPaymentMethod === 'credits' ? 'border-emerald-600 bg-emerald-50 text-emerald-600 font-black' : 'border-slate-100 text-slate-400'}`}
                    >
                      <Coins size={16} />
...
const MenuManager: React.FC<{ location: LocationData; onUpdateLocation: (updates: Partial<LocationData> | ((prev: LocationData) => Partial<LocationData>)) => void; userRole: string; }> = ({ location, onUpdateLocation, userRole }) => {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setItemImg(reader.result as string);
      };
  ...
  const handleToggleItemAvailability = (id: string, e: React.MouseEvent, currentStatus: boolean) => {
    e.stopPropagation();
    onUpdateLocation(prev => {
      const prevMenu = prev.menu || [];
      return { menu: prevMenu.map(m => m.id === id ? { ...m, isAvailable: !currentStatus } : m) };
  ...
  const handleSaveItem = () => {
    ...
    onUpdateLocation(prev => {
      ...
      if (editingItem) {
        ...
        return { menu: [...prevMenu, { id: `item-${Date.now()}`, name: itemName, price: itemPrice, description: itemDesc, category: activeCategory.name, isAvailable: true, image: itemImg || 'https://images.unsplash.com/photo-1541167760496-1628856ab772?auto=format&fit=crop&q=80&w=400' }] };
      }
    });
    setEditingItem(null);
          ...
          {activeCategory ? (
            <div className="max-w-6xl space-y-8">
               <div className="flex items-center justify-between">
                 ...
                 {canEditMenu && !isAddingItem && !editingItem && (
                   <button 
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {currentCategoryItems.map(item => {
                    ...
                    return (
                      <div key={item.id} className={`bg-white rounded-3xl p-5 border flex flex-col group relative transition-all ${isAvailable ? 'border-slate-200' : 'border-slate-100 opacity-60 grayscale-[50%]'}`}>
                         <div className="absolute top-2 right-2 z-10 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                           <button 
                             onClick={(e) => handleToggleItemAvailability(item.id, e, isAvailable)}
                             className="w-8 h-8 bg-white/90 backdrop-blur text-slate-600 rounded-full flex items-center justify-center shadow-sm hover:bg-slate-100"
                           >
                           ...
                           {canEditMenu && (
                             <button 
                               ...
                               className="w-8 h-8 bg-white/90 backdrop-blur text-rose-500 rounded-full flex items-center justify-center shadow-sm hover:bg-rose-500 hover:text-white"
                             >
                               <X size={14} />
                             </button>
                           )}
                         ...
                         <div className="aspect-[4/3] rounded-2xl overflow-hidden mb-4 relative bg-slate-100">
                            ...
                            {!isAvailable && (
                              <div className="absolute inset-0 bg-slate-900/40 flex items-center justify-center backdrop-blur-[2px]">
                                <span className="bg-slate-900 text-white font-black text-[10px] uppercase tracking-widest px-3 py-1 rounded-full">Unavailable</span>
                              </div>
                            )}
                         </div>
