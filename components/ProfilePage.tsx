...
import { User, Mail, Sparkles, Globe, Camera, Check, Lock, CreditCard, Plus, Trash2, LogOut, X, Edit2, Briefcase } from 'lucide-react';
...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, reservations, onLogout, onUpdateProfile, onClose, onNavigateToCredits }) => {
  ...
  const handleSave = () => {
    setIsSaving(true);
    setTimeout(() => {
      onUpdateProfile({ ...formData, paymentMethods });
      setIsSaving(false);
      setShowSuccess(true);
            ...
            <div className="pt-2">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-4">
                ...
                <button 
                  ...
                  className={`px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-[10px] font-black uppercase tracking-widest flex items-center gap-1 ${onNavigateToCredits ? 'hover:bg-emerald-100 transition-colors cursor-pointer' : ''}`}
                >
                  <Sparkles size={12} /> {(formData.credits || 0).toLocaleString()} Nova Points
                </button>
              </div>
            </div>
          </div>
